...
### 4.6. Quản lý Nhân sự (Staff Management)

Manager có thể quản lý các tài khoản nhân viên (Phục vụ, Lễ tân, Thu ngân...) làm việc tại nhà hàng của mình.

#### 4.6.1. Lấy danh sách nhân viên
**`GET /api/v1/manager/staffs`**

| Thuộc tính | Giá trị |
| --- | --- |
| **Summary** | Lấy danh sách nhân viên đang làm việc tại nhà hàng do Manager quản lý |
| **Auth** | Yes (Bearer Token) |
| **Role** | MANAGER |
...
| `page` | `integer` | No | `0` | Số trang |
| `limit` | `integer` | No | `10` | Số phần tử/trang |
| `role` | `string` | No | `null` | Lọc theo chức vụ (WAITER, RECEPTIONIST, CASHIER) |
  ...
  "data": [
    {
      ...
      "email": "waiter01.res15@nhahang.local",
      "phone": "0901234567",
      "role": "WAITER",
      "status": "ACTIVE"
    }
...
#### 4.6.2. Tạo tài khoản nhân viên
**`POST /api/v1/manager/staffs`**

| Thuộc tính | Giá trị |
| --- | --- |
| **Summary** | Tạo tài khoản nhân viên mới cho nhà hàng. Hệ thống tự gán `workplace` = Nhà hàng hiện tại. |
| **Auth** | Yes (Bearer Token) |
| **Role** | MANAGER |
...
```json
{
  ...
  "data": {
    ...
    "username": "waiter02",
    "fullName": "Nguyễn Văn Phục Vụ 2",
    "email": "waiter02.res15@nhahang.local",
    "phone": "0987654321",
    "role": "WAITER",
    "status": "ACTIVE",
    "workplaceRestaurantId": 15
  }
}
```
...
```json
{
  ...
  "targetRole": "ALL", // Có thể là "ALL", "CUSTOMER", "MANAGER"
  "type": "SYSTEM_ALERT"
}
```
