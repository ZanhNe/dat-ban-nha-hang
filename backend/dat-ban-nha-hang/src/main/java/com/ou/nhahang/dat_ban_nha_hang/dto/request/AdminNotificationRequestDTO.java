...
public record AdminNotificationRequestDTO() {
    ...
    public record Broadcast(
            ...
            @Pattern(regexp = "^(ALL|CUSTOMER|MANAGER|RECEPTIONIST|WAITER|CASHIER|ADMIN)?$", message = "targetRole không hợp lệ")
            String targetRole,

            @NotBlank(message = "Loại thông báo không được để trống")
            @Pattern(regexp = "^(SYSTEM_ALERT|SYSTEM_MESSAGE|PROMOTION)$", message = "Loại thông báo không hợp lệ")
            String type
    ) {
    }
